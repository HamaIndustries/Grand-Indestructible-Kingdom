...
public class VerticalCardboardBlock extends CardboardBlock {
    ...
    protected BlockState getStateForNeighborUpdate(BlockState state, WorldView world, ScheduledTickView tickView, BlockPos pos, Direction direction, BlockPos neighborPos, BlockState neighborState, Random random) {
        return neighborState.getFluidState().isIn(FluidTags.WATER) ? GIK.SOAKED_VERTICAL_CARDBOARD.getStateWithProperties(state) : state;
    }

    @Override
    protected Block getSoakedVersion() {
        return GIK.SOAKED_VERTICAL_CARDBOARD;
    }
}
